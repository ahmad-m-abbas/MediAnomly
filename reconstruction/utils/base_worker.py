...
class BaseWorker:
    ...
    def set_seed(self):
        ...
        self.seed = 42 #np.random.randint(1, 999999)
    ...
    def set_network_loss(self):
        if self.opt.model['name'] in ['ae', 'ceae', 'ae-ssim', 'ae-l1', 'ae-perceptual']:
            ...
            if self.opt.model['name'] == 'ae-ssim':
                self.criterion = SSIMLoss(beta=self.opt.train['beta'], grad_pen_weight=self.opt.train['grad_pen_weight'])
            elif self.opt.model['name'] == 'ae-l1':
                self.criterion = L1Loss(beta=self.opt.train['beta'], grad_pen_weight=self.opt.train['grad_pen_weight'])
            elif self.opt.model['name'] == 'ae-perceptual':
                self.criterion = RelativePerceptualL1Loss(beta=self.opt.train['beta'], grad_pen_weight=self.opt.train['grad_pen_weight'])
            else:
                self.criterion = AELoss(beta=self.opt.train['beta'], grad_pen_weight=self.opt.train['grad_pen_weight'])
            ...
            self.net = AE(input_size=self.opt.model['input_size'], in_planes=self.opt.model['in_c'],
                          ...
                          mid_num=self.opt.model['hidden_num'], latent_size=self.opt.model['ls'],
                          en_num_layers=self.opt.model["en_depth"], de_num_layers=self.opt.model["de_depth"])
            self.criterion = AELoss(beta=self.opt.train['beta'], grad_pen_weight=self.opt.train['grad_pen_weight'], grad_flag=True)
            ...
            self.net = ConstrainedAE(input_size=self.opt.model['input_size'], in_planes=self.opt.model['in_c'],
                                     ...
                                     mid_num=self.opt.model['hidden_num'], latent_size=self.opt.model['ls'],
                                     en_num_layers=self.opt.model["en_depth"], de_num_layers=self.opt.model["de_depth"])
            self.criterion = ConstrainedAELoss(beta=self.opt.train['beta'], grad_pen_weight=self.opt.train['grad_pen_weight'])
        elif self.opt.model['name'] == 'memae':
            self.net = MemAE(input_size=self.opt.model['input_size'], in_planes=self.opt.model['in_c'],
                             base_width=self.opt.model['base_width'], expansion=self.opt.model['expansion'],
                             mid_num=self.opt.model['hidden_num'], latent_size=self.opt.model['ls'],
                             en_num_layers=self.opt.model["en_depth"], de_num_layers=self.opt.model["de_depth"])
            self.criterion = MemAELoss(beta=self.opt.train['beta'], grad_pen_weight=self.opt.train['grad_pen_weight'])
        elif self.opt.model['name'] == 'aeu':
            self.net = AEU(input_size=self.opt.model['input_size'], in_planes=self.opt.model['in_c'],
                           base_width=self.opt.model['base_width'], expansion=self.opt.model['expansion'],
                           mid_num=self.opt.model['hidden_num'], latent_size=self.opt.model['ls'],
                           en_num_layers=self.opt.model["en_depth"], de_num_layers=self.opt.model["de_depth"])
            self.criterion = AEULoss(beta=self.opt.train['beta'], grad_pen_weight=self.opt.train['grad_pen_weight'])
            ...
            if self.opt.model['name'] == 'vae':
                self.criterion = VAELoss(beta=self.opt.train['beta'], grad_pen_weight=self.opt.train['grad_pen_weight'])
            elif self.opt.model['name'] == 'vae-elbo':
                self.criterion = VAELoss(beta=self.opt.train['beta'], grad_pen_weight=self.opt.train['grad_pen_weight'], grad='elbo')
            elif self.opt.model['name'] == 'vae-kl':
                self.criterion = VAELoss(beta=self.opt.train['beta'], grad_pen_weight=self.opt.train['grad_pen_weight'], grad='kl')
            elif self.opt.model['name'] == 'vae-rec':
                self.criterion = VAELoss(beta=self.opt.train['beta'], grad_pen_weight=self.opt.train['grad_pen_weight'], grad='rec')
            elif self.opt.model['name'] == 'vae-combi':
                self.criterion = VAELoss(beta=self.opt.train['beta'], grad_pen_weight=self.opt.train['grad_pen_weight'], grad='combi')
            else:
