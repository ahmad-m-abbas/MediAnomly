...
class AEWorker(BaseWorker):
    ...
    def evaluate(self):
        ...
        for idx_batch, data_batch in enumerate(self.test_loader):
            ...
            img = img.cuda()
            img.requires_grad = True #self.grad_flag  # Will be True for gradient-based methods

            net_out = self.net(img)

            if 'z' in net_out and not net_out['z'].requires_grad:
                 net_out['z'] = net_out['z'].requires_grad_()
            anomaly_score_map = self.criterion(img, net_out, anomaly_score=True, keepdim=True)[0].detach().cpu()
    ...
    def run_train(self):
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            self.logger.log(step=epoch, data={"train/recon_loss": recon_loss})
            self.logger.log(step=epoch, data={"train/embedding_loss": embedding_loss})
            self.logger.log(step=epoch, data={"train/grad_penalty": grad_penalty})
